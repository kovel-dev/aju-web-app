...
const OrganizationFormSchema = async (enteredData, isUpdatePage) => {
  ...
  if (isUpdatePage) {
    ...
    validationSchema['accepted_term_and_conditions'] = Yup.string().oneOf(
      ['yes'],
      {
...
export const validateOrganizationForm = async (
  ...
  return await OrganizationFormSchema(enteredData, isUpdatePage).then(
    async (response) => {
      return response
