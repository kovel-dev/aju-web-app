export const JoinWaitlistConfirmation = (data) => {
  return {
    from: `'"${process.env.NOTIFICATION_NAME}" <${process.env.NOTIFICATION_EMAIL}>'`,
    to: data.user.email,
    subject: 'Waitlist Confirmation: ' + data.program.name,
    text: `
    <!DOCTYPE html>
    <html lang="en">
    <head>
    ...
    <body style="max-width: 900px; margin: 0 auto;">
      <main style="border: 15px solid #E3E3E3;">
        <div style="padding: 50px 8%">
          ...
          <div>
            <h1 style="text-align: center; font-size: 24px; border-top: 1px solid #979797; border-bottom: 1px solid #979797; color: #084E85; padding: 15px 0; margin: 40px auto; max-width: 460px;">You've been added <span style="white-space: nowrap;">to the waitlist</span></h1>
          </div>
          <div>
            <img src="${data.program.imageUrl}" alt="" style="max-width: 100%;">
          </div>
          <h2 style="color: #084E85; font-size: 20px; margin-top: 40px;">Hi ${data.user.first_name},</h2>
          <p style="color: #161D25; font-size: 16px; line-height: 30px;">Thanks for joining the waitlist for ${data.program.name}. We appreciate your interest! We will be in touch with you as soon as a spot opens up.</p>
          <h2 style="color: #084E85; font-size: 20px; margin-top: 40px;">About Us</h2>
          <p style="color: #161D25; font-size: 16px; line-height: 30px;">As your online Jewish learning hub, we offer online, in-person and on-demand classes on Jewish art, culture, literature and more.</p>
