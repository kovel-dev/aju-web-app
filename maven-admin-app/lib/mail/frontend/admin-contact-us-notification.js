export const AdminContactUsNotification = (data) => {
  return {
    from: `'"${process.env.NOTIFICATION_NAME}" <${process.env.NOTIFICATION_EMAIL}>'`,
    to: process.env.ADMIN_RECIPIENTS.split(','),
    subject: 'Contact Us Submission Notification',
    text: `
    <p>New Contact Us Submission from ${data.name} with email of ${data.email}.</p>
    <p>Name: ${data.name}</p>
    <p>Email: ${data.email}</p>
    <p>Phone: ${data.mobile_number}</p>
    <p>Contact By Phone?: ${data.method_of_contact_phone}</p>
    <p>Message: ${data.message}</p>
    <p><a href="${process.env.NEXT_PUBLIC_APP_URL}" target="_blank">Go to AJU Maven Admin</a></p>
    `,
    html: `
    <p>New Contact Us Submission from ${data.name} with email of ${data.email}.</p>
    <p>Name: ${data.name}</p>
    <p>Email: ${data.email}</p>
    <p>Phone: ${data.mobile_number}</p>
    <p>Contact By Phone?: ${data.method_of_contact_phone}</p>
    <p>Message: ${data.message}</p>
    <p><a href="${process.env.NEXT_PUBLIC_APP_URL}" target="_blank">Go to AJU Maven Admin</a></p>
    `,
  }
}
