...
function ListOfTags() {
  ...
  const buttonHandler = (ref, type) => {
    setIsLoading(true)
    if (type == 'edit') {
      router.push(`${server}/tags/${ref}/edit`)
  ...
  const confirmedChangeStatusHandler = async () => {
    ...
    try {
      if (statusChange == 'active') {
        await tag.activate(toChangeStatusItem).then(async (response) => {
          setIsLoading(false)
        ...
        await tag.deactivate(toChangeStatusItem).then(async (response) => {
          setIsLoading(false)
          setToChangeStatusItem(null)
          setStatusChange(null)
          await updatePage(1, numOfItemsPerPage)
  ...
  return (
    <div className="container-fluid mt-5">
      ...
      <div className="d-grid gap-2 d-md-flex justify-content-md-end my-2">
        <Cx.CButton href={`${server}/tags/create`} disabled={isLoading}>
          Create Tag
        </Cx.CButton>
      </div>
      <Cx.CCard className="w-auto ">
        <Cx.CCardHeader>List of Tags</Cx.CCardHeader>
        <Cx.CCardBody>
          <Cx.CTable>
            <Cx.CTableHead>
              <Cx.CTableRow>
              ...
              {tagList.map((tag, index) => {
                return (
                  <Cx.CTableRow key={index}>
                    <Cx.CTableDataCell>{tag.name}</Cx.CTableDataCell>
                    <Cx.CTableDataCell>{tag.is_featured}</Cx.CTableDataCell>
                    <Cx.CTableDataCell>{tag.status}</Cx.CTableDataCell>
                    <Cx.CTableDataCell>
                      <Cx.CButton
                        color="secondary"
                        onClick={() => buttonHandler(tag.ref, 'edit')}
                        disabled={isLoading}
                      >
                        Edit
                      </Cx.CButton>
                      {tag.status == 'ACTIVE' && (
                        <Cx.CButton
                          color="danger"
                          onClick={() => buttonHandler(tag.ref, 'inactive')}
                          disabled={isLoading}
                        >
                          Deactivate
                        </Cx.CButton>
                      ...
                      {tag.status == 'INACTIVE' && (
                        <Cx.CButton
                          color="success"
                          onClick={() => buttonHandler(tag.ref, 'active')}
                          disabled={isLoading}
                        >
                          Activate
                        </Cx.CButton>
