...
export default NextAuth({
  ...
  providers: [
    Providers.Credentials({
      async authorize(credentials) {
        ...
        if (!isValid) {
          // Add error logging here
          throw new Error('(ERC 100) Invalid credentials. Please try again.')
        }

        //check if user is active or not
        if (user.data.status != 'active') {
          throw new Error('(ERC 200) Invalid credentials. Please try again.')
        }

        //check if user is deleted
        if (user.data.is_deleted === true) {
          throw new Error('(ERC 300) Invalid credentials. Please try again.')
        }

        //check if user is verified
        if (user.data.is_verified === false) {
          throw new Error('(ERC 400) Invalid credentials. Please try again.')
        }

        if (['developer', 'admin', 'staff'].indexOf(user.data.role) == -1) {
          throw new Error('(ERC 500) Invalid credentials. Please try again.')
        }
        ...
        if (user.data.role == 'partner') {
          try {
            ...
            throw new Error('(ERC 600) Invalid credentials. Please try again.')
          }
  ...
  callbacks: {
    ...
    async session(session, token) {
      ...
      if (token && token.userRefID !== undefined) {
        let isAdminUser =
          ['developer', 'admin', 'staff'].indexOf(token.userRole) > -1

        if (isAdminUser) {
          // for Admin user return a normal session
