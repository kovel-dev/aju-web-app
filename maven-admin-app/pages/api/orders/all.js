...
async function handler(req, res) {
  if (req.method == 'GET') {
    ...
    const count = await faunaClient.query(
      q.Count(q.Match(q.Index('orders_desc')))
    )

    await faunaClient
      .query(
        q.Paginate(q.Match(q.Index('orders_desc')), {
          size: count,
        })
      )
      .then(async (result) => {
        ...
        if (resultSet.length > 0) {
          for (let index = 0; index < resultSet.length; index++) {
            const order = resultSet[index]
            const userId = order[17]
            ...
            try {
              await faunaClient
                ...
                .then((ret) => {
                  ...
                  order.push(userData.first_name + ' ' + userData.last_name)

                  const formattedCreatedAt = moment(order[0])
                    .local()
                    .format('YYYY-MM-DD HH:mm:ss')
                  order.push(formattedCreatedAt)

                  newResultSet.push(order)
                })
            } catch (error) {
              // @TODO: log error
            }
