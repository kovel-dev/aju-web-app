...
async function handler(req, res) {
  ...
  if (req.method == 'POST') {
    ...
    userData.cart_meta.map(async (item, index) => {
      ...
      if (parseInt(newProductProfileData.capacity) < item.capacity) {
        res.status(422).json([
          {
            key: newProductProfile.ref.id,
            values: `${newProductProfile.data.name} has already reached it's maximum capacity`,
          },
        ])
        return
      }
    ...
    // VERIFY PROMO CODES AVAILABILITY
    ...
    promoCodes.map(async (item, index) => {
      ...
      if (promoCodeObj.data.products_meta.length > 0) {
        ...
        promoCodeObj.data.products_meta.map((item, index) => {
          ...
          userData.cart_meta.map((cartItem, cartIndex) => {
